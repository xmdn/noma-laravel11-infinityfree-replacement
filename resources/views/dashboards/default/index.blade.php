<div class="store" x-data="{ menu: false }" @keydown.escape.window="$wire.cartOpen = false; menu = false">
    ...
    <header class="nav-shell">
        ...
        <a class="wordmark" href="/">{{ $shopName }}<span>®</span></a>
        <nav :class="menu && 'open'" aria-label="Main navigation">
            <a href="#shop" @click="menu = false">New arrivals</a>
            @foreach (array_slice($this->categories, 1, 2) as $navCategory)
                <a href="#shop" wire:click="$set('category', '{{ $navCategory }}')" @click="menu = false">{{ $navCategory }}</a>
            @endforeach
            <a href="#journal" @click="menu = false">Journal</a>
        </nav>
        <div class="nav-actions">
            <a href="{{ auth()->check() ? url('/dashboard') : url('/login') }}" style="color:inherit;text-decoration:none;font-size:12px;font-weight:500">Account</a>
    ...
    <main id="top">
        <section class="hero-store">
            ...
            <div class="hero-content"><p class="kicker">{{ $shopName }} · 2026</p><h1>Objects for<br>a considered life.</h1><p>Essential forms, honest materials, and enduring function. Designed to live with you, not around you.</p><a class="shop-link" href="#shop">Explore the collection <span>↘</span></a></div>
        ...
        <section class="catalog" id="shop">
            ...
            <div class="catalog-tools">
                ...
                <div class="tool-right">
                    <label class="catalog-search"><span>⌕</span><input id="catalog-search" name="catalog_search" wire:model.live.debounce.250ms="query" placeholder="Search objects" aria-label="Search products"></label>
                    <label class="sort" for="catalog-sort"><span>Sort</span><select id="catalog-sort" name="catalog_sort" wire:model.live="sort"><option value="featured">Featured</option><option value="newest">Newest</option><option value="price-asc">Price: low</option><option value="price-desc">Price: high</option></select></label>
                </div>
            </div>
        ...
        <section class="newsletter">
            ...
            @else
                <form wire:submit="subscribe"><div><input id="newsletter-email" name="email" type="email" autocomplete="email" wire:model="email" placeholder="Email address" aria-label="Email address"><button type="submit">Join us <span>→</span></button></div>@error('email')<small>{{ $message }}</small>@enderror</form>
            @endif
        </section>
    </main>
